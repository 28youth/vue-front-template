<template>
  <span v-if="anonymity" :class="styles" class="m-avatar-box anonymity">
    {{ $t('hide') }}
  </span>
  <RouterLink v-else :to="path" :class="styles" class="m-avatar-box c-avatar" @click="viewUser">
    <img :src="avatar" class="m-avatar-img" @error="handelError" />
    <i v-if="icon" :style="icon" :class="iconClass" class="m-avatar-icon" />
  </RouterLink>
</template>
