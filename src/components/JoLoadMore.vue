  ...
  >
    ...
    >
      <slot name="head">
        ...
        <i v-else :style="{ transform: `rotate(${topStatus ? '180deg' : '0'})` }" class="jo-loadmore-icon" />
        <span>{{ topTxt }}</span>
      </slot>
    </div>
    <!-- body -->
    <div :style="{ transform: `translateY(${tY}px)`, transitionDuration }" class="jo-loadmore-main">
      <slot />
      <!-- footer -->
      <div
...
export default {
  ...
  methods: {
    ...
    stopDrag() {
      ...
      this.tY > this.maxDistance && window.scrollY <= 0 ? this.beforeRefresh() : (this.dY = 0)
    },
    beforeRefresh() {
    ...
    afterRefresh(noMore = true) {
      ...
      this.$nextTick(() => {
        noMore || (this.showBottom && this.autoLoad && !this.isFulled() && this.beforeLoadMore())
      })
    },
    beforeLoadMore() {
    ...
    isFulled() {
      return this.$el.getBoundingClientRect().bottom >= this.scrollTarget.clientHeight
    }
  }
}
</script>
