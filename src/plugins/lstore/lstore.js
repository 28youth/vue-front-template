...
export default {
  isSupported: (() => {
    try {
      return window.sessionStorage.setItem('PlusTest', 'lalala'), window.sessionStorage.removeItem('PlusTest'), true
    } catch (err) {
  ...
  removeData(key, session = false) {
    const store = session ? 'sessionStorage' : 'localStorage'
    this.isSupported && window[store] && window[store][key] && window[store].removeItem(key)
  },
  ...
  getCookie(key) {
    ...
    let cookie
    return document.cookie.length > 0 && (cookie = document.cookie.match(reg)) ? unescape(cookie[2]) : null
  },
